...
最近 commits：
  3463079 防惡意灌號：雲端取號服務網址（含通行碼）改為只存各裝置本機，「發布上線」時自動剔除絕不進 public.json——鑰匙不再貼在門上；欄位提示改為請幹部間私下傳網址、各裝置貼一次
  d023cf6 自動更新 CLAUDE.md 最近進度（Stop hook）
  654726d 雲端共用取號系統：org.js 新增雲端取號服務介接（quhaoUrl 設定＋peek/next/log API），文號流水簿與公文產生器在服務啟用時改走雲端共用計數器（多人同時取號不撞號、發文簿存雲端試算表隨時可查），未設定或斷線時維持本機模式並明確警示、絕不靜默改用本機計數；並修正 sw.js 會攔截跨網域請求的嚴重問題（雲端取號回應若被快取，每次會拿到同一個舊號碼）——跨網域一律不攔、僅保留兩個既有 CDN 例外
  eacfa38 自動更新 CLAUDE.md 最近進度（Stop hook）
  6e81b05 自動更新 CLAUDE.md 最近進度（Stop hook）
<!-- AUTO:END -->
