...
最近 commits：
  512b76b 會員名冊：新增加密備份/匯入(密碼保護，可安心存進工會共用雲端，沒密碼打開只有亂碼)；會員身分連動職稱自動帶入(正式會員→契約護理師、贊助會員→公職護理師，已手動改過的不覆蓋)；每位會員自動產生固定4碼「繳費代碼」供總務轉帳/ATM對帳比對，並可用代碼搜尋、CSV匯出
  6e445ee 自動更新 CLAUDE.md 最近進度（Stop hook）
  014943c 自動更新 CLAUDE.md 最近進度（Stop hook）
  c4a39c2 會員名冊：新增「🔁 疑似重複」篩選(同姓名多筆自動抓出來)；公職人員自動判定關鍵字擴大(公務人員/公務員/公教人員等)，不再只認「公職」兩字；新增批次刪除功能(勾選多筆一次刪、可從最近刪除復原)
  dc805c2 自動更新 CLAUDE.md 最近進度（Stop hook）
<!-- AUTO:END -->
