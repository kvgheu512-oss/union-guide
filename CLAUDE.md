...
最近 commits：
  04e005b 會員名冊：待審核的人不再顯示醒目綠色「正式會員」標籤，改成「申請中／擬：」灰色標示且狀態標籤排最前面，避免誤以為還沒核准就已經是正式會員
  1c011ea 自動更新 CLAUDE.md 最近進度（Stop hook）
  2495625 自動更新 CLAUDE.md 最近進度（Stop hook）
  49e5dae 自動更新 CLAUDE.md 最近進度（Stop hook）
  61e07ea 合併列印：名冊空的時候加「去會員名冊新增」按鈕直接跳轉；收據金額快選改為點哪個就直接帶入(移除手動鎖定判斷)、經辦人改回下拉選單並加「🔄換人」鍵可清空重選；會員名冊：連署名單貼上匯入改為可更新既有資料(依時間戳記取最新)、新增公職人員安全鎖(勾選/偵測後強制鎖定贊助會員避免觸法)、核准入會前檢查是否已有繳費紀錄
<!-- AUTO:END -->
