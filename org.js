...
(function () {
  ...
  // 台灣本地日期（YYYY-MM-DD）。注意：不要用 new Date().toISOString().slice(0,10)——
  // 那是 UTC 日期，台灣時間每天 00:00~08:00（UTC 前一天）會少算一天，
  // 公文日期、發文簿等以「今天」為準的地方都要用這支。
  ...
  var FIELDS = [
    ...
    ["beikeUrl", "貝殼集器繳款連結", "申請開通貝殼集器後把收款連結貼這裡並發布；繳費方式頁（pay.html）會自動顯示線上繳費按鈕。未開通前留空"],
    ["joinSheetUrl", "入會申請書自動回傳網址", "部署「入會申請自動寫入 Google Sheet」服務後，把網址貼這裡並發布；join-form.html 送出申請時會自動回傳，不用再靠會員手動傳連結。這欄會公開（寫在網頁原始碼裡讓所有申請人都連得到），不是機密，未部署前留空"]
  ];
  ...
  function nextDocNo(subject, series) {
    ...
    try {
      ...
      log.push({ no: no, date: localDateStr(), subject: subject || "", series: k.seq === "docSeqPrep" ? "prep" : "main" });
